# GoshaCrash 3.10.2-rc36

**rc36:** на чистой установке `config.yaml` теперь сразу создаётся как готовая безопасная DIRECT-заглушка одним UTF-8 heredoc — без промежуточного ASCII-файла, AWK и последующего «украшения». Русские комментарии находятся прямо в `install.sh`, поэтому `/bin/sh` записывает их байт-в-байт. Для существующего конфига по-прежнему сохраняются YAML-данные, старые битые комментарии удаляются и создаются заново в UTF-8. Финальный файл обязательно проверяет сам Mihomo через `-t`.

# GoshaCrash

## 3.10.2-rc36 — готовая UTF-8 заглушка формируется сразу
