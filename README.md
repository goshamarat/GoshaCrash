...
### Самый простой способ подготовить флешку на Windows
...
- Размер раздела: весь USB-диск

После Rufus вставь флешку в ASUS и запусти:

```sh
sh /tmp/install.sh --prepare-usb
```

Если ASUSWRT видит один `/dev/sdX1`, занимающий практически весь физический USB-диск,
GoshaCrash **не пересоздаёт MBR и не запускает fdisk**. Он размонтирует этот раздел,
форматирует именно его в EXT3 и проверяет Unix symlink. В штатном Rufus-сценарии reboot
для форматирования не нужен.

### Логика определения разметки

`--prepare-usb` различает три случая:

1. **Есть ровно один `/dev/sdX1`** — самый частый случай. `fdisk` вообще не используется; существующий раздел размонтируется и форматируется в EXT3.
2. **Разделов нет, filesystem записана прямо на `/dev/sdX`** — создаётся MBR + один `/dev/sdX1`, после чего выполняется EXT3. Если старое ядро ещё держит прежнее отображение устройства, мастер не запускает `mkfs` на stale mapping и просит переподключить USB/перезагрузить роутер.
3. **Несколько разделов** — автоматическое destructive-форматирование запрещается; пользователь должен явно привести флешку к одному разделу.

Обычная FAT/TFAT/NTFS-флешка с уже существующим `/dev/sda1` поэтому форматируется **без reboot и без изменения partition table**.
