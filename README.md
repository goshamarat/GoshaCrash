...
## 3.10.2-rc30 — диагностика TUN и упрощение меню
...
## 3.10.2-rc30 — полный повторный аудит rc28
...
- При interrupted update Zashboard rc30 умеет восстановить предыдущий валидный UI из временного swap-каталога; после нормального завершения временных UI-каталогов не остаётся.
...
rc30 собирает в одну версию исправления BT10, меню и структуры каталогов, а также закрывает несколько проблем обновления:
...
Перед online install в `main` должны быть одновременно загружены **оба** файла rc30: `install.sh` и `goshacrash.sh`. Если одно зеркало ещё отдаёт старый controller, installer попробует следующее; несовпадающая версия не устанавливается.
