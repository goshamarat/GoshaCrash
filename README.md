# rc38 — детерминированный UTF-8 config
...
- `--reset-config` явно заменяет текущий `config.yaml` новой UTF-8 DIRECT-заглушкой. Старый файл держится только во временном `/tmp` до успешного `mihomo -t`, затем удаляется.
- Если Mihomo после удаления комментариев выдаёт уже другую YAML-ошибку, installer показывает её как YAML-ошибку, а не ошибочно называет повреждённым UTF-8.

## Готовая UTF-8 заглушка формируется сразу

- На чистой установке installer одним `cat <<EOF` сразу пишет конечный `config.yaml` с русскими комментариями, TUN, DNS, API, Zashboard и правилом `MATCH,DIRECT`.
- Нет промежуточного `config-base-data`, нет AWK-прохода и нет отдельной стадии добавления комментариев для свежего файла. Это убирает лишнее место, где могла повредиться кириллица.
- Заглушка безопасная: до добавления пользовательских proxy/group/rules весь трафик идёт `DIRECT`, но сам TUN/DNS runtime уже описан корректно.
- Файл создаётся UTF-8 без BOM и затем всё равно проверяется встроенным parser'ом установленного Mihomo до запуска runtime.
- Для upgrade существующего пользовательского `config.yaml` данные не уничтожаются: старые whole-line комментарии удаляются, GoshaCrash-owned поля нормализуются, а канонические русские комментарии создаются заново.

- Старые whole-line комментарии удаляются **до** любых shell/AWK-миграций. Это исключает перенос CP1251/обрезанных UTF-8 байтов в новый файл.
- Пользовательские YAML-данные (`proxies`, `proxy-groups`, `rule-providers`, `rules`, DNS-значения и остальные поля) не перекодируются и не удаляются.
- После нормализации TUN/DNS/API/UI installer заново добавляет подробные комментарии на русском языке. Исходник `install.sh` и итоговый `config.yaml` — UTF-8 без BOM.
- Все AWK-проходы, которые читают или переписывают `config.yaml`, принудительно запускаются с `LC_ALL=C`, чтобы ASUS BusyBox обрабатывал файл как байты и не зависел от локали прошивки. То же сделано для runtime-переключения routing mode.
- Итоговый config проверяется реальным установленным Mihomo через `mihomo -t`. Если YAML без комментариев валиден, а вариант со свежими русскими комментариями вдруг нет, installer считает это внутренней ошибкой и останавливается — без fallback на безкомментарный config.
- Постоянные backup-файлы не создаются: исходный пользовательский config хранится только во временном `/tmp` до успешной проверки.

## 3.10.2-rc34 — защита config.yaml от повреждённой кодировки

- Перед миграцией существующего `config.yaml` installer проверяет реальные байты UTF-8.
- Если повреждены только старые строки-комментарии (типичный CP1251/обрезанный UTF-8), installer удаляет только эти комментарии, добавляет свежий UTF-8 заголовок и сохраняет все YAML-данные.
- Если не-UTF-8 байты находятся внутри реальных YAML-данных, installer ничего не конвертирует наугад, возвращает исходный файл из временной копии `/tmp` и останавливается с понятной ошибкой.
- Свежесгенерированный config дополнительно проверяется на UTF-8 сразу после создания.
- `gc check` и `gc doctor` теперь отдельно диагностируют кодировку config до запуска parser Mihomo.

## 3.10.2-rc34 — полный аудит формирования config.yaml

- Свежий `config.yaml` теперь формируется напрямую, без последующего awk-патча уже созданного TUN-блока. Поэтому `auto-route`, `auto-redirect` и `auto-detect-interface` сразу стоят на своих местах внутри `tun:` и комментарии не съезжают между секциями.
- Проверены оба базовых варианта: modern `auto` (`stack: system`, auto-route/redirect/detect = true, без `routing-mark`) и manual/legacy (`auto-* = false`, `routing-mark: 9012`; на ARMv5 используется `stack: gvisor`).
- В базовый конфиг добавлен `find-process-mode: off`: документация Mihomo рекомендует отключать process matching на роутерах.
- При upgrade старый пользовательский конфиг сохраняет `proxies`, `proxy-groups`, `rules`, resolver lists и остальные пользовательские поля, но GoshaCrash нормализует свои обязательные TUN/DNS/API/UI настройки.
- Если старый конфиг вообще не содержит `tun:` или `dns:`, installer создаёт полноценные минимальные блоки, включая `dns-hijack`, Fake-IP и resolver defaults, а не только отдельные `enable/listen` поля.
- `external-controller` приводится к `0.0.0.0:9090`, `external-ui` — к `ui`, `external-ui-url` — к Zashboard; отсутствующий/пустой/`CHANGE_ME` secret заменяется уникальным локальным secret.
- Ambiguous YAML больше не правится вслепую: duplicate `tun:`/`dns:` и flow-style `tun: {...}`/`dns: {...}` останавливают безопасную миграцию до изменения файла.
- Миграция существующего config теперь транзакционная: исходник хранится только во временном `/tmp` до успешного `mihomo -t`; при любой ошибке installer восстанавливает исходный файл. Постоянный backup не создаётся.
- `gc check` дополнительно проверяет ожидаемый `tun.stack`, наличие `tun.dns-hijack`, `auto-detect-interface`, API secret и согласованность routing mode.


## 3.10.2-rc34 — диагностика TUN и упрощение меню
...
## 3.10.2-rc34 — полный повторный аудит rc28
...
- При interrupted update Zashboard rc34 умеет восстановить предыдущий валидный UI из временного swap-каталога; после нормального завершения временных UI-каталогов не остаётся.
...
rc34 собирает в одну версию исправления BT10, меню и структуры каталогов, а также закрывает несколько проблем обновления:
...
Перед online install в `main` должны быть одновременно загружены **оба** файла rc34: `install.sh` и `goshacrash.sh`. Если одно зеркало ещё отдаёт старый controller, installer попробует следующее; несовпадающая версия не устанавливается.
