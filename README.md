...
- Убрана ненадёжная проверка Info-ZIP по тексту `unzip -h`.
- Старый Optware `/opt/bin/unzip-unzip` принимается напрямую и проверяется уже реальной распаковкой Zashboard.
- Если `ipkg` считает `unzip` установленным, но бинарника нет, установщик один раз автоматически переустанавливает пакет.
- `nano` остаётся обязательной зависимостью и ставится автоматически.
